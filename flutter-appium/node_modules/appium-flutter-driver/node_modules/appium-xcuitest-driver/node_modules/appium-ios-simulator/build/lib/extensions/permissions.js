...
async function execSQLiteQuery(db, query) {
    logger_1.default.debug(`Executing SQL query "${query}" on '${db}'`);
    try {
...
async function execWix(args) {
    ...
    logger_1.default.debug(`Executing: ${WIX_SIM_UTILS} ${support_1.util.quote(args)}`);
    try {
...
async function getAccess(bundleId, serviceName, simDataRoot) {
    ...
    const dbPath = path_1.default.resolve(simDataRoot, 'Library', 'TCC', 'TCC.db');
...
extensions.getPermission = async function getPermission(bundleId, serviceName) {
    ...
    logger_1.default.debug(`Got ${serviceName} access status for '${bundleId}': ${result}`);
    return result;
};
