    ...
    if (fullReset) {
        ...
        if (app) {
            ...
            try {
                ...
                logger_1.default.warn(`Reset: could not scrub ${isSafari ? 'Safari browser' : 'application with id "' + opts.bundleId + '"'}. ` + `Leaving as is.`);
            }
        }
...
async function shutdownOtherSimulators(currentDevice) {
    ...
    logger_1.default.info(`Detected ${otherBootedDevices.length} other running ${support_1.util.pluralize('Simulator', otherBootedDevices.length)}.` + `Shutting them down...`);
